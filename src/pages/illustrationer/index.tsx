...
import { ILLUSTRATIONER_CARD_TYPES } from "./CardTypes";

/** Illustrationer - AI-genererade illustrationer */
export default function Illustrationer() {
	return (
		<div style={styles.wrapper}>
			<div style={styles.container}>
				<h1 style={styles.title}>Illustrationer</h1>
				
				<div style={styles.scrollContainer}>
					{ILLUSTRATIONER_CARD_TYPES.map((cardType) => (
...
function CardComponent({ cardType }: { cardType: any }) {
	...
	useEffect(() => {
		const savedKeyword = localStorage.getItem(`illustrationer_keyword_${cardType.id}`);
		const savedSystemPrompt = localStorage.getItem(`illustrationer_systemPrompt_${cardType.id}`);
		const savedCode = localStorage.getItem(`illustrationer_code_${cardType.id}`);
	...
	const saveData = () => {
		localStorage.setItem(`illustrationer_keyword_${cardType.id}`, keyword);
		localStorage.setItem(`illustrationer_systemPrompt_${cardType.id}`, systemPrompt);
		alert(`${cardType.title} sparad`);
	};

	const saveCode = () => {
		localStorage.setItem(`illustrationer_code_${cardType.id}`, codeInstructions);
		alert("Kod sparad");
	};
