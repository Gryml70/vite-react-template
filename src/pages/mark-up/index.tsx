...
import { MARKUP_CARD_TYPES } from "./CardTypes";

/** Mark Up - Schema Markup */
export default function MarkUp() {
	return (
		<div style={styles.wrapper}>
			<div style={styles.container}>
				<h1 style={styles.title}>Mark Up</h1>
				
				<div style={styles.scrollContainer}>
					{MARKUP_CARD_TYPES.map((cardType) => (
...
function CardComponent({ cardType }: { cardType: any }) {
	...
	useEffect(() => {
		const savedKeyword = localStorage.getItem(`markup_keyword_${cardType.id}`);
		const savedSystemPrompt = localStorage.getItem(`markup_systemPrompt_${cardType.id}`);
		const savedCode = localStorage.getItem(`markup_code_${cardType.id}`);
	...
	const saveData = () => {
		localStorage.setItem(`markup_keyword_${cardType.id}`, keyword);
		localStorage.setItem(`markup_systemPrompt_${cardType.id}`, systemPrompt);
		alert(`${cardType.title} sparad`);
	};

	const saveCode = () => {
		localStorage.setItem(`markup_code_${cardType.id}`, codeInstructions);
		alert("Kod sparad");
	};
